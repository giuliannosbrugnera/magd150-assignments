...
void draw() {
  ...
  if ( playerLost() ) {
    // Ellipse reached right border. Draw random ellipses
    drawRandomEllipses();
    noLoop();
  }
  else {
    ...
    String message;
    if ( posX < width / 2 ) {
      message = "The ellipse is before the half of the screen! Click to keep it away from the right border!";
    }
    else if ( posX < width - 300 ) {
      message = "Do not let the ellipse reach the right border! Keep clicking!";
    }
    else {
      message = "Oh no... Behold!";
    }
...
// Draw the moving ellipse
void drawMovingEllipse() {
...
// Draw ten random setted up ellipses
